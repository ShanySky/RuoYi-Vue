...
每次实际工作前先识别当前执行上下文，并确认与任务有关的当前事实，不依赖历史会话或静态快照：

- 当前是在本地工作区、GitHub / 云端、CI Runner，还是用户明确指定的远程环境。
- 目标仓库、目标分支以及当前 Working Tree / 远端分支状态。
- `chatgpt/ai-agent-assistant` 与 `master` 的共享远端关系；需要判断共同主线最新状态时再核实 GitHub。
- 最新相关代码、配置、SQL、测试和 workflow。
- 本任务是否存在独立讨论 / 实施分支。

不要因为某个 Connector、SSH、本地工作区或远程工具“可用”就自行切换执行环境。执行上下文、能力缺失和跨环境边界统一遵守：

- `.agents/rules/execution-context.md`
...
- 前后端联动修改在当前能力允许时必须同时检查 `ShanySky/RuoYi-Vue3` 的协议和当前实现；如果当前上下文无法访问配套仓库，只阻塞真正依赖该事实的步骤，不编造结果，也不因此擅自切换环境。
- 能自动完成的验证尽量在当前执行上下文内自动完成；重要 Agent 改动优先做真实编译、启动、接口、数据库和端到端验证。
...
先判断当前任务实际建立在哪个执行上下文，再依据真实能力选择本地命令、GitHub API、Actions、远程环境或其他执行方式；不要从“工具存在”反推已经获得跨环境授权。
...
- `doc/ai/05-产品规划与商业化/26-AI助手产品愿景与商业化路线.md`

该文档是三层产品愿景、版本演进、商业化路线和“数字员工”正式术语的 canonical source。讨论 V1、商业模式、合作伙伴材料、投资人材料和长期产品路线时，以该文档为产品战略基线，再结合执行时最新代码和阶段文档。
