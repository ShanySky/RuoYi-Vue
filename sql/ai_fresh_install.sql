-- RuoYi AI Assistant canonical fresh-install schema (Phase 3.5 P0)
...
create table if not exists ai_pending_tool_call (
  ...
  reasoning_effort  varchar(16)     default null,
  run_id             bigint(20)      default null,
  capability_protocol varchar(64)     default null,
...
('/system/user','用户管理','0','system',sysdate()),('/system/user-auth/role/*','分配角色','0','system',sysdate()),('/system/role','角色管理','0','system',sysdate()),('/system/menu','菜单管理','0','system',sysdate()),('/system/dept','部门管理','0','system',sysdate()),('/system/post','岗位管理','0','system',sysdate()),('/system/dict','字典类型','0','system',sysdate()),('/system/notice','通知公告','0','system',sysdate()),('/monitor/online','在线用户','0','system',sysdate()),('/monitor/job','定时任务','0','system',sysdate()),('/monitor/logininfor','登录日志','0','system',sysdate()),('/monitor/operlog','操作日志','0','system',sysdate()),('/monitor/cache','缓存监控','0','system',sysdate()),('/monitor/cacheList','缓存列表','0','system',sysdate()),('/monitor/server','服务监控','0','system',sysdate()),
