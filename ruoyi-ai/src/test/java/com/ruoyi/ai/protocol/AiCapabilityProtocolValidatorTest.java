...
class AiCapabilityProtocolValidatorTest
{
    ...
    {
        AiChatTurnRequest longInstance = validRequest();
        longInstance.setPageInstanceId("x".repeat(65));
        assertThrows(ServiceException.class, () -> validator.validateRequest(longInstance));

        AiChatTurnRequest relativeRoute = validRequest();
        relativeRoute.setRoute("system/user");
        assertThrows(ServiceException.class, () -> validator.validateRequest(relativeRoute));

        AiChatTurnRequest longPageId = validRequest();
        longPageId.setPageId("x".repeat(129));
    ...
    void continuationMustMatchPendingSemanticPageSnapshot()
    {
        ...
        AiChatTurnRequest staleVersion = validRequest();
        staleVersion.setPageVersion(2L);
        assertThrows(ServiceException.class, () -> validator.validateContinuation(staleVersion, pending, true));
    }
