    ...
    public SystemToolPolicyProvider()
    {
        ...
        values.put("page_system_user_search", new ToolPolicyDefinition(
                        ...
                        objectSchema(Map.of(
                                ...
                                List.of())));
        values.put("page_system_user_reset", new ToolPolicyDefinition(
                        "READ", "system:user:list", "重置当前用户管理页查询条件并刷新数据",
                        objectSchema(Map.of(), List.of())));
        values.put("page_system_user_add_open", new ToolPolicyDefinition(
                        "UI", "system:user:add", "在当前用户管理页打开新增用户表单",
                        objectSchema(Map.of(), List.of())));
        values.put("page_system_user_add_set_fields", new ToolPolicyDefinition(
                        "UI", "system:user:add", "填写当前新增用户表单的受控字段但不保存",
                        objectSchema(Map.ofEntries(
                                Map.entry("userName", Map.of("type", "string", "description", "登录账号")),
                                Map.entry("password", Map.of("type", "string", "description", "初始密码")),
                                Map.entry("nickName", Map.of("type", "string", "description", "用户昵称")),
                                Map.entry("deptId", Map.of("type", "integer", "description", "归属部门ID")),
                                Map.entry("phonenumber", Map.of("type", "string", "description", "手机号码")),
                                Map.entry("email", Map.of("type", "string", "description", "邮箱")),
                                Map.entry("sex", Map.of("type", "string", "enum", List.of("0", "1", "2"), "description", "性别")),
                                Map.entry("status", Map.of("type", "string", "enum", List.of("0", "1"), "description", "状态")),
                                Map.entry("postIds", Map.of("type", "array", "items", Map.of("type", "integer"), "description", "岗位ID列表")),
                                Map.entry("roleIds", Map.of("type", "array", "items", Map.of("type", "integer"), "description", "角色ID列表")),
                                Map.entry("remark", Map.of("type", "string", "description", "备注"))),
                                List.of())));
        values.put("page_system_user_add_submit", new ToolPolicyDefinition(
                        "WRITE", "system:user:add", "提交当前新增用户表单并真实写入系统",
                        objectSchema(Map.of(), List.of())));
