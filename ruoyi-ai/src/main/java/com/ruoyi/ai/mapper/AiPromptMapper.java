...
public interface AiPromptMapper {
 ...
 @Update("update ai_prompt set content=#{content},version_no=version_no+1,update_by=#{username},update_time=sysdate() where prompt_type=#{type}") int updateContent(@Param("type")String type,@Param("content")String content,@Param("username")String username);
 @Update("update ai_prompt set content=default_content,version_no=version_no+1,update_by=#{username},update_time=sysdate() where prompt_type=#{type}") int restoreDefault(@Param("type")String type,@Param("username")String username);
}