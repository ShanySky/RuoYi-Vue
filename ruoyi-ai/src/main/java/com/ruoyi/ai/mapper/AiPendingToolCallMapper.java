...
{
    @Insert("insert into ai_pending_tool_call(call_id, conversation_id, user_id, tool_name, arguments_json, risk_level, model_id, model_code, reasoning_effort, status, create_time, expire_time) "
            + "values(#{callId}, #{conversationId}, #{userId}, #{toolName}, #{argumentsJson}, #{riskLevel}, #{modelId}, #{modelCode}, #{reasoningEffort}, 'PENDING', sysdate(), date_add(sysdate(), interval 10 minute))")
    ...
    @Select("select pending_id as pendingId, call_id as callId, conversation_id as conversationId, user_id as userId, "
            + "tool_name as toolName, arguments_json as argumentsJson, risk_level as riskLevel, model_id as modelId, "
            + "model_code as modelCode, reasoning_effort as reasoningEffort, status, create_time as createTime, expire_time as expireTime, resolved_time as resolvedTime "
