...
{
    @Insert("insert into ai_conversation(user_id, model_id, title, route, status, create_time, update_time) "
            + "values(#{userId}, #{modelId}, #{title}, #{route}, #{status}, sysdate(), sysdate())")
